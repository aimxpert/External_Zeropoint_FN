...
namespace uee
{
	...
	public:
		...
		camera_position_s get_camera()
		{
			...
			uintptr_t location_pointer = read<uintptr_t>(arrays->uworld + 0x128);
			uintptr_t rotation_pointer = read<uintptr_t>(arrays->uworld + 0x138);
			FNRot fnrot{};
		...
		bool is_visible(uintptr_t mesh, float tolerance = 0.06f) {
			...
			float LastRenderTime = read<float>(mesh + 0x2F0);
